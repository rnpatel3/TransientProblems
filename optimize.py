...
class MultiObj(om.ExplicitComponent):
    def initialize(self):
        pass
    def setup(self):
        self.add_input('topo_c')
        self.add_input('topo_stress')
        self.add_output('Multi_obj')
        self.declare_partials(of='Multi_obj', wrt='topo_c')
        self.declare_partials(of='Multi_obj', wrt='topo_stress')
...
def optimize(in_picklename, in_opt_problem, in_optimizer, in_qvals, in_epsilon,
    ...
    for in_qval in in_qvals:
        ...
        if in_opt_problem == 'comp_min_mass_constr':
        ...
        elif in_opt_problem == 'stress_min_comp_max_mass_constr':
            ...
            prob.model.add_subsystem('topo_comp',MultiObj())
            prob.model.connect('topo.c','topo_comp.topo_c')
            prob.model.connect('topo.ks_nodal_stress','topo_comp.topo_stress')
            prob.model.add_objective('topo_comp.Multi_obj', scaler=1.0)
            # Doesn't work : prob.model.add_subsystem('topo_comp', om.ExecComp(topo_c='topo.ks_nodal_stress'-'topo.c'))
            # Maybe works: prob.model.add_subsystem('topo_comp', subsys=MultiObj(), promotes_inputs=['topo.c','topo.ks_nodal_stress'])
            #prob.model.add_objective('topo.ks_nodal_stress', scaler=1.0)
            
            
            #prob.model.add_objective('topo_comp', scaler = 1)
            prob.model.add_constraint('topo.m', upper=full_mass*in_design_mass)
            ...
            # Specify type of stress relexation
            if in_ParOpt_use_qn_correction:
                stress_type = 'a'
            else:
                stress_type = 'm'
            ...
            if in_stress_as_fraction:
                analysis = Analysis(conn, dof, X, force, r0,
                    C, density, in_qval, in_epsilon, in_ks_parameter, stress_type=stress_type)
                x_full = np.ones(nnodes)
        ...
        if in_ParOpt_use_qn_correction:
            ...
            if 'comp' in in_opt_problem:
                if in_opt_problem == 'comp_min_massfreq_constr':
                    prob.driver.use_qn_correction(analysis.freq_constr_qn_correction)
                else:
                    prob.driver.use_qn_correction(analysis.compliance_obj_qn_correction)

            elif in_opt_problem == 'mass_min_stress_constr':
                prob.driver.use_qn_correction(analysis.stress_constr_qn_correction)

            # This technique is only used for compliance problems
            elif in_opt_problem == 'stress_min_mass_constr':
                print("\n[Warning] ParOpt_use_qn_correction is switched on for an unsupported problem!")
    ...
    if in_opt_problem == 'comp_min_mass_constr':
        ...
        obj = prob.get_val('topo_comp.Multi_obj')[0]
        con1 = prob.get_val('topo.m')[0] / (full_mass*in_design_mass) - 1.0
        cons = [con1]
