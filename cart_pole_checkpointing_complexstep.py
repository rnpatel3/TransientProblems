...
class CartPole():
    ...
    def evalObjCon(self, x):
        ...
        #fobj = self.fobj_scale*np.sum(self.h*x[:]**2) Doesn't work for CS test, see line below
        fobj = self.fobj_scale*np.sum(x[:]**2)
    ...
    def evalObjConGradient(self, x):
        """Evaluate the objective and constraint gradient"""
        fail = 0

        # The objective gradient
        g = 2.0*self.fobj_scale*x[:]
        A = np.zeros((4,40))

        # Create a vector to store the derivative of the constraints
        dfdx = np.zeros(self.t.shape, dtype=x.dtype)
    ...
    def computeTrajectory(self, t, u, start, end, store=False):
        ...
        #q[0,:] = 0.0
        q_i_prev = np.zeros((1,4), dtype=u.dtype)
        #checkpoints = np.arange(0,40)
                                
        
        # Compute the residual and Jacobian
        res = np.zeros(4, dtype=u.dtype)
        J = np.zeros((4, 4), dtype=u.dtype)
...
print("Relative error CS, Constraints: ", (analytical_A-cs_con)/cs_con)
